...
class adsOn: UIView {
    ...
    override func layoutSubviews() {
        mainScroll.frame = self.bounds
        label1.frame = CGRect(x: 0, y: 0, width: self.bounds.width, height: self.bounds.height)
        label2.frame = CGRect(x: self.bounds.width, y: 0, width: self.bounds.width, height: self.bounds.height)
    }
    ...
    override init(frame: CGRect) {
        ...
        mainScroll.autoresizingMask = [.FlexibleWidth,.FlexibleHeight]
